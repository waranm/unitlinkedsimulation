...
| Fee hook | `applyFees(` ใน `js/fees.js` | ✅ implemented (Phase 2b — admin fee live) |
...
| Admin fee deduction (pro-rata) | `adminFeeRate`, `applyFees` returns `{adminFee, lapsed}` | ✅ implemented (Phase 2b) |
| Lapse detection + tracking | `lapseMonth`, `survivalMonthly`, `survivalYearly` | ✅ implemented (Phase 2b) |
| Snapshot AUM after premium, before fee | comment `SNAPSHOT AUM` ใน scenario loop | ✅ implemented (Phase 2b) |
| Buy at OFFER, value at BID (preserved) | `offerRatio` ใน premium block, `bidRatio` ใน snapshot/value | ✅ preserved |
| Market shock at end of month | comment `Market shock` ที่ปลาย loop | ✅ reordered (Phase 2b) |
| In-force percentile filter | `isActiveAt`, `lapseMonths[s] > m` ใน runMonteCarlo | ✅ implemented (Phase 2b) |

Quick check command:
```bash
grep -cE "bidRatio|offerRatio|applyFees|adminFeeRate|lapseMonth|survivalMonthly|SNAPSHOT AUM|Market shock" js/simulation.js
# Phase 2b baseline ≈ 60; ถ้าตกต่ำกว่านี้ = regression
```

### 🚧 Planned — NOT yet implemented

- **Phase 2c:** COI deduction (TMO2560 lookup, NAR/SA basis), premium charge (year-based rate, payment-count policy year)
- ไฟล์อ้างอิง: `docs/thai-mortality-2560-custom.xlsx` มี COI table จริงจาก insurer

### ✅ Done & verified

- Phase 1: GBM + Bid/Offer + rebalance frequencies (78/78 tests pass)
- Phase 2a: product config + COI plumbing + policy info UI
- Phase 2a.5: term handling — `coverageMonths` + PPT-capped premium + IRR fix
- Phase 2b: admin fee + lapse detection + survival curve (103/103 tests pass, verified 2026-04-25)
...
❌ **Post-lapse zero values leaking into percentile/mean** (Phase 2b)
หลัง lapse, `values[m] = 0` เป็น **internal marker** ห้ามแสดงตรงๆ หรือนำเข้า percentile/mean

ถ้าใช้ raw `allSeries[s][m]` ใน percentile sort → 0 จะดึง median/P25 ลงผิด

**Pattern ที่ถูก:** filter ก่อน aggregate
```js
const isActiveAt = (s, m) => lapseMonths[s] === null || lapseMonths[s] > m;
const active = [];
for (let s = 0; s < N; s++) if (isActiveAt(s, m)) active.push(allSeries[s][m]);
// percentile/mean ทำกับ `active` เท่านั้น
```

❌ **Premium intake / fee / market shock ทำงานหลัง lapse** (Phase 2b)
ต้องมี `if (lapseMonth !== null) { values[m] = 0; continue; }` **บนสุด** ของ for-loop
ห้ามวางหลังจุดใดจุดหนึ่ง — ไม่งั้นบางขั้นจะ execute ทำให้ portfolio "ฟื้น" ผิดธรรมชาติ

❌ **D1 loop order** (Phase 2b) — ลำดับใน scenario loop ที่ห้ามสลับ:
1. Premium → buy at OFFER
2. Compute `bidPrices` snapshot (after premium, before fees)
3. `applyFees(portfolio, bidPrices, feeParams, m)` → `{ adminFee, lapsed }`
4. Rebalance (ถ้าตรงกำหนด)
5. Record `values[m]` at BID
6. Market shock — NAV update **for next month**

ลำดับนี้ทำให้ valuation ของเดือน m ไม่รวม shock ของเดือน m เอง — เป็น semantic ใหม่ของ Phase 2b

❌ **Bash session cwd ไม่ sync กับ EnterWorktree** (worktree pitfall #2)
EnterWorktree switch session-level worktree แต่ Bash subprocess อาจ inherit cwd
จาก parent session เดิม — โดยเฉพาะหลัง hook fire (launch preview, background tasks)
หรือเริ่ม message ใหม่

อาการ: Edit/Write (ใช้ absolute path) ลงไฟล์ถูก worktree
แต่ `node tools/test-simulation.js`, `python -m http.server`, `git status`
รันใน worktree เก่าโดยไม่รู้ตัว → tests "ผ่าน" บนโค้ดที่ยังไม่ได้แก้, server serve ไฟล์ผิด

**Defense-in-depth ที่ใช้จริง:**

1. ก่อน destructive/long-running bash → verify cwd ก่อน 1 วินาที:
   ```bash
   pwd && git branch --show-current
   ```

2. Long-running processes → pin ด้วย absolute path / `--directory`:
   ```bash
   python -m http.server 8080 --directory "/abs/path/to/correct/worktree"
   node /abs/path/to/worktree/tools/test-simulation.js
   ```

3. หลัง edit เสร็จ — verify ผ่าน absolute path เสมอ:
   ```bash
   grep -n "<keyword>" /abs/path/to/worktree/js/file.js
   ```
   อย่าใช้ relative `grep file.js` ทันทีหลัง edit cross-worktree

4. ถ้าเจอ test ผ่านแบบน่าสงสัย (ผ่านทุกเทสต์รวมถึงเทสต์ใหม่ที่เพิ่งเขียน) →
   เช็ค `pwd` ทันที อาจรันผิด dir
